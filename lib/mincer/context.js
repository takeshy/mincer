...
// cached (when possible) template evaluation
function evaluateTemplate(template, self, pathname, locals, callback) {
  var options = {}, klass = template.constructor;

  if (!self.environment.cacheEnabled || !template.constructor.cachable) {
    template.evaluate(self, locals, callback);
    return;
  }

  options.id            = klass.__name__ || klass.name;
  options.pathname      = pathname;
  options.dependencies  = pathname === self.pathname ? self.__dependencyAssets__
                        : [pathname];

  self.environment.cacheGet(function (callback) {
    template.evaluate(self, locals, callback);
  }, options, callback);
}
...
Context.prototype.evaluate = function (pathname, options, callback) {
  ...
  async.forEachSeries(processors, function (ProcessorKlass, next) {
    ...
    evaluateTemplate(template, self, pathname, locals, function (err, data) {
      if (err) {
